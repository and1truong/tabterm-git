// Build the git module into two self-contained runtime artifacts under dist/:
//   src/index.tsx -> dist/modules/git/client.js   (ESM, react external)
//   server.ts     -> dist/modules/git/server.js   (ESM, --target bun)
//
// A single-module distillation of the tabterm host's scripts/build-modules.ts.
// Like the timer module, git imports no CSS and does no dynamic import(), so
// there is no CSS extraction and no code-splitting: each half is one flat file.
// (highlight.js — used by the diff view — is a plain JS dep, so it inlines into
// client.js rather than being externalized.) Run from the repo root.
...
const OUT = join(REPO, "dist", "modules", "git");
...
// host-shims), so the client bundle keeps them external. highlight.js is not
...
console.log(`[build] git → ${join("dist", "modules", "git")}/{client.js,server.js}`);
