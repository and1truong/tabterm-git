...
install: ## Install deps (highlight.js/lucide-react + vendored module-host)
	$(BUN) install

typecheck: ## Type-check the module against the host contract
	$(BUN) run typecheck

test: ## Run the git + subtree service tests
	$(BUN) test

check: typecheck test ## Type-check then test (the full local gate)

build: ## Build self-contained dist/modules/git/{client.js,server.js}
